    ...
    manager.mount(duelWorld, {
      ...
      callbacks: {
        onTargetChange: target => {
          ...
          if (!target) {
            const requiredDevice = getDuelRequiredDevice();
...
function renderDuelChoices() {
  ...
  if (!duelPanelDevice) {
    ...
    duelActionTitle.textContent = localPlayerRole === "hacker" ? "Walk to your gear" : "Walk to your workstation";
    duelActionSubtitle.textContent = localPlayerRole === "hacker"
      ...
      : "Use WASD to move, click to lock your mouse, and press E near your office computer or work phone.";
    renderDuelDeviceViewport();
    return;
  }
  ...
  duelActionTitle.textContent = scenario
    ...
    : "Wait for the attacker to launch something believable";
  duelActionSubtitle.textContent = scenario
    ...
    : "The office side stays idle until a lure reaches your workflow.";
...
function renderDuelArena() {
  ...
  if (duelRoundLabel) {
    duelRoundLabel.textContent = duelState ? `Round ${duelState.round}` : "Round 1";
  }
  if (duelRoleLabel) {
  ...
  if (duelState && duelState.finished) {
    ...
    setDuelStatus(duelState && duelState.selectedAttackId
      ...
      : `Pick an attack to open the round. ${duelState ? duelState.hackerCover : 0} cover left.`);
  } else {
    setDuelStatus(duelState && duelState.selectedAttackId
      ? `A live lure reached your workstation. ${duelState.officeIntegrity} recovery chances left.`
      : `Watching the office workflow for the next threat. ${duelState ? duelState.officeIntegrity : 0} recovery chances left.`);
  }
...
function joinDuelRoom() {
  ...
  if (savedProgress) {
    localPlayerName = savedProgress.localPlayerName || localPlayerName;
    duelState = savedProgress.duelState || createDuelState();
  } else {
    duelState = createDuelState();
  }
