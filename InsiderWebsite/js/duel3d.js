(function () {
  ...
  class Duel3DManager {
    ...
    mount(container, options = {}) {
      ...
      this.touchMode = Boolean(options.touchMode);
    ...
    setRole(role) {
      ...
      this.mount(this.container, { role, playerRole: this.playerRole, callbacks: this.callbacks, touchMode: this.touchMode });
    }
    ...
    detachEvents() {
      if (this.renderer) {
        ...
        this.renderer.domElement.removeEventListener("touchcancel", this.boundTouchEnd);
      }
    ...
    onTouchStart(event) {
      if (!this.touchMode || !event.changedTouches.length) return;
    ...
    onTouchMove(event) {
      if (!this.touchMode || !this.touchLookActive || this.touchLookId === null) return;
      const touch = Array.from(event.changedTouches).find(entry => entry.identifier === this.touchLookId);
      if (!touch) return;
      event.preventDefault();
      this.yaw -= (touch.clientX - this.lastTouchX) * 0.005;
      this.pitch -= (touch.clientY - this.lastTouchY) * 0.0042;
      this.pitch = Math.max(-1.05, Math.min(1.05, this.pitch));
      this.lastTouchX = touch.clientX;
      this.lastTouchY = touch.clientY;
    }
    ...
    onTouchEnd(event) {
      if (!this.touchMode || this.touchLookId === null) return;
      const touch = Array.from(event.changedTouches).find(entry => entry.identifier === this.touchLookId);
      if (!touch) return;
      this.touchLookActive = false;
      this.touchLookId = null;
    ...
    triggerJump() {
      if (this.movementLocked || !this.onGround) return;
      this.jumpVelocity = 4.9;
      this.onGround = false;
    }

    triggerInteract() {
      if (this.activeTarget && typeof this.callbacks.onInteract === "function") {
        this.callbacks.onInteract(this.activeTarget.type);
      }
    }

    triggerTag() {
      if (this.tagEnabled && this.canTagPartner && typeof this.callbacks.onTagAttempt === "function") {
        this.callbacks.onTagAttempt();
      }
    }
